...
function LinearInterpolationRegularGrid(u::AV,t₀::T=0,t₁::T=length(u),Δt::T=1) where {T<:Number,AV<:AbstractVector{<:Number}}
    n = length(u)
    @assert length(t₀:Δt:t₁-Δt) == n
...
function LinearInterpolationRegularGrid(U::AV,t₀::T=0,t₁::T=size(U,2),Δt::T=1) where {T<:Number,AV<:AbstractMatrix{<:Number}}
    n = size(U,2) 
    @assert length(t₀:Δt:t₁-Δt) == n
    t = collect(eltype(U),t₀:Δt:t₁-Δt)
    # u, t = munge_data(U, t)
...
function ConstantInterpolationRegularGrid(u::AV,t₀::T=0,t₁::T=length(u),Δt::T=1) where {T<:Number,AV<:AbstractVector{<:Number}}
    n = length(u)
    @assert length(t₀:Δt:t₁-Δt) == n
    t = collect(t₀:Δt:t₁-Δt)
    # u, t = munge_data(u, t)
...
function ConstantInterpolationRegularGrid(U::AV,t₀::T=0,t₁::T=size(U,2),Δt::T=1) where {T<:Number,AV<:AbstractMatrix{<:Number}}
    n = size(U,2)
    @assert length(t₀:Δt:t₁-Δt) == n
    t = collect(t₀:Δt:t₁-Δt)
    # u, t = munge_data(U, t)
...
function DataInterpolations.CubicSpline(u::AbstractMatrix{<:Number},t::AbstractVector{<:Number})
    ...
    d[:,2:end-1].=6diff(u,dims=2)[:,2:end]./Δt[:,2:end] - 6diff(reverse(u,dims=2),dims=2)[:,1:end-1]./Δt[:,1:end-1]
    z = permutedims(tA\permutedims(d))
    CubicSpline{true}(u,vec(t),reshape(h[1:n+1],1,:),z)
end
# Base.rpad(v::AbstractVector, n::Integer, p) = [v; fill(p, max(n - length(v), 0))]

function Base.rpad(v::AbstractMatrix, n::Integer, p; dims)
  ds = size(v) |> collect
  ds[dims] = max(n-ds[dims])
  ds = Tuple(ds)
...
function DataInterpolations.CubicSpline(u::U,t::T) where {U<:Vector{<:AbstractVector{<:Number}},T<:Vector{<:AbstractVector{<:Number}}}
    ...
    tmp_right = 6right(diff.(reverse.(u)));
